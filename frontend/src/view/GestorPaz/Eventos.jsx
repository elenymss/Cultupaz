...
function Eventos() {
  ...
  const [tema_evento, setTitulo] = useState('');
  ...
  const editarEvento = async (evento) => {
    ...
    Swal.fire({
      ...
      html: `
        ...
        <div class="form-group">
          <label>Fecha:</label>
          <input   type="date" id="swal-input3" class="form-control">${evento.fecha_evento}</input>
        </div>
      `,
      showCancelButton: true,
      ...
      preConfirm: () => {
        ...
        const fecha_evento = Swal.getPopup().querySelector('#swal-input3').value;

        return { tema_evento, descripcion_evento, fecha_evento };
      },
    }).then((result) => {
      if (result.isConfirmed) {
        const { tema_evento, descripcion_evento, fecha_evento } = result.value;
        actualizarEvento(evento.idEvento, tema_evento, descripcion_evento, fecha_evento);
      }
    });
  };

  const actualizarEvento = async (idEvento, nuevoTitulo, nuevaDescripcion, nuevaFecha) => {
    try {
